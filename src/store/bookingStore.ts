  ...
  persist(
    (set, get) => ({
      ...
      validatePassengers: () => {
        ...
        // Only update validation errors when explicitly validating (not during render)
        // This prevents infinite loops when called from getters like getStepProgress
      ...
      addBaggage: (passengerId, weight, price) =>
        set((state) => {
          ...
          baggage.push({ passengerId, type: 'checked', weight, price })
          return {
      ...
      toggleInsurance: () =>
        set((state) => ({
          extras: {
            ...state.extras,
            insurance: state.extras.insurance ? undefined : {
              enabled: true,
              coverage: {
      ...
      setPaymentMethod: (method) =>
        set((state) => ({
          payment: { ...state.payment, method: method as any },
          isDirty: true,
        })),
      ...
      validateStep: (step) => {
        ...
        switch (step) {
          ...
          case 'passengers':
            // Basic check without calling validatePassengers to avoid setState during render
      ...
      getStepProgress: () => {
        ...
        // Don't call validateCurrentStep here as it can cause infinite loops
        // Just return progress based on completed steps
