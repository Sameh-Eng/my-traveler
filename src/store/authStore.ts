    ...
    (set, get) => ({
        ...
        try {
          ...
          if (response.success) {
            set({
              ...
              twoFactorMethod: method,
              user: get().user ? { ...get().user, twoFactorEnabled: true } : null,
              isLoading: false,
            })
          } else {
            throw new Error(response.error?.message || 'Failed to enable two-factor authentication')
          }
        } catch (error) {
        ...
        try {
          ...
          if (response.success) {
            set({
              ...
              twoFactorMethod: null,
              twoFactorBackupCodes: [],
              user: get().user ? { ...get().user, twoFactorEnabled: false } : null,
              isLoading: false,
            })
          } else {
      ...
      verifyEmail: async (token) => {
        ...
        try {
          ...
          if (response.success) {
            set({
              user: get().user ? { ...get().user, isVerified: true } : null,
              isVerifyingEmail: false,
              verificationToken: null,
              isLoading: false,
            })
      ...
      updateProfile: async (updates) => {
        ...
        try {
          ...
          if (response.success) {
            set({
              user: get().user ? { ...get().user, profile: { ...get().user!.profile, ...updates } } : null,
              isLoading: false,
            })
          } else {
      ...
      updatePreferences: async (preferences) => {
        ...
        try {
          ...
          if (response.success) {
            set({
              user: get().user ? { ...get().user, preferences: { ...get().user!.preferences, ...preferences } } : null,
              isLoading: false,
            })
          } else {
      ...
      uploadAvatar: async (file) => {
        ...
        try {
          ...
          if (response.success) {
            set({
              user: get().user ? { ...get().user, profile: { ...get().user!.profile, avatar: response.data.url } } : null,
              isLoading: false,
            })
          } else {
      ...
      isSessionExpired: () => {
        ...
        return new Date(lastActivity.getTime() + 30 * 60 * 1000) <= new Date()
      },
