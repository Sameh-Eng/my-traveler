...
const PaymentPage = () => {
  ...
  const handlePaymentSubmit = async (paymentData: PaymentFormData) => {
    try {
      ...
      // In a real app, this would process the payment via Stripe or other payment processor
      const paymentResult = {
        success: true,
      ...
      if (paymentResult.success) {
        ...
        router.push(`/booking/confirmation?bookingId=${bookingId}&transactionId=${paymentResult.transactionId}`)
      } else {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className={`flex items-center space-x-2 px-3 py-2 rounded-lg ${
              paymentTimeLeft < 300 ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
            }`}>
                      ...
                      <div key={passenger.id || index} className="flex items-center justify-between">
                        <div>
                          <div className="font-medium">
                            {passenger.basicInfo.firstName} {passenger.basicInfo.lastName}
                          </div>
                          <div className="text-sm text-gray-600 capitalize">{passenger.type}</div>
                        </div>
                        <Badge variant="secondary">{passenger.basicInfo.email}</Badge>
                      </div>
                    ))}
                  </div>
                </CardContent>
