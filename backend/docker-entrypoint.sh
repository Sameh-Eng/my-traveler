...
# Start nginx in background (if nginx is installed)
if command -v nginx >/dev/null 2>&1; then
    echo "🌐 Starting Nginx..."
    nginx -t
    nginx -g "daemon on;"
else
    echo "ℹ️  Nginx not found, running Node.js server only"
fi
...
if health_check; then
    ...
    echo "📖 API Documentation: http://localhost:${PORT:-3001}/api/paymob"
else
